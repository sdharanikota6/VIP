...
def get_sentiment(transcript):
    ...
    for chunk in transcript_chunks:
        ...
        if 'star' in sentiment_result['label']:
            score = int(sentiment_result['label'].split()[0])  # Extract the number before "star"
            sentiment_scores.append(score)
        else:
            print(f"Unexpected label format: {sentiment_result['label']}")

    # Decide final sentiment based on the average score
    average_score = np.mean(sentiment_scores) if sentiment_scores else 3  # Default to neutral if no scores
...
company_name_mapping = {
    'JP Morgan': 'JP',
    'UnitedHealth Group': 'UnitedHealth'  # Modify as necessary based on your directory structure
}
...
for index, row in df.iterrows():
    ...
    if os.path.isfile(transcript_path):
        ...
        df.at[index, 'Average_Score'] = np.mean(scores) if scores else 'No Score'
    else:
